...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedActions_requestFeedFromLoader() {
        ...
        XCTAssertEqual(loader.loadFeedCallCount, 1, "Expected no request until first request has completed.")
    ...
    func test_loadMoreActions_requestMoreFromLoader() {
        ...
        XCTAssertEqual(loader.loadMoreCallCount, 0, "Expect no loading request when view is initialized.")
        ...
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 3, "Expected no after loading all pages")
    }
    ...
    func test_loadMoreIndicator_isVisibleWhileLoadingMore() {
        ...
        sut.simulateLoadMoreFeedAction()
        XCTAssertTrue(sut.isShowingLoadMoreFeedIndicator, "Expected laoding indicator on load more action.")
    ...
    func test_feedImageView_DoesNotLoadImageAgainUntilPreviousRequestCompletes() {
        ...
        sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image0.url], "Expected no request until previous request completes")
        
        loader.completeImageLoading(at: 0)
        sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image0.url], "Expected second request when visible after previous completes")
        
        sut.simulateFeedImageViewNotVisible(at: 0)
        sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image0.url, image0.url], "Expected Expected third request when visible after canceling previous complete")
    }
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        loader.cancelImageLoadingWithError(at: 1)
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator state change for first view once second image loading completes with error")
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, false, "Expected no loading indicator for second view once second image loading completes with error")
    }
    
    func test_feedImageViewLoadingIndicator_rendersImageLoadedFromURL() {
        let (sut, loader) = makeSUT()
        ...
        XCTAssertEqual(view1?.renderedImage, .none, "Expected no image state change for second view once first image laoding completes successfully")
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() {
        ...
        XCTAssertEqual(view1?.isShowingRetryAction, false, "Expected no retry action state change for second view once first image laoding completes successfully")
    ...
    func test_feedImageViewRetryButton_isVisibleOnInvalidImageData() {
        ...
        XCTAssertEqual(view?.isShowingRetryAction, true, "Expected retry action once image loading completes with invalod image data.")
    }
    ...
    func test_feedImageViewRetryAction_retriesImageLoad() {
        ...
        loader.cancelImageLoadingWithError(at: 0)
        loader.cancelImageLoadingWithError(at: 1)
