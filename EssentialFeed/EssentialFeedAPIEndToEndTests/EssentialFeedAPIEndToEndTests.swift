...
//
//  Created by Nikhil Menon on 11/15/23.
//
...
final class EssentialFeedAPIEndToEndTests: XCTestCase {
    ...
    private func getFeedResult(file: StaticString = #file, line: UInt = #line) -> FeedLoader.Result? {
        ...
        let loader = RemoteLoader(url: testServerURL, client: ephemeralClient(), mapper: FeedItemsMapper.map(_:from:))
        ...
        wait(for: [exp], timeout: 5.0) // may have to bump the timeout based on server lag.
