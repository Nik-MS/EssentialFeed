    ...
    func loadImageData(from url: URL, completion: @escaping (FeedImageDataLoader.Result) -> Void) -> FeedImageDataLoaderTask {
        ...
        task.wrapped = client.get(from: url) { [weak self] result in
            ...
            case let .failure(clientError):
                task.complete(with: .failure(clientError))
            }
        }
...
final class RemoteFeedImageDataLoaderTests: XCTestCase {
    ...
    func test_loadImageDataFromURL_deliversInvalidDataErrorOnNon200HTTResponse() {
        let (sut, client) = makeSUT()
    ...
    func test_loadImageDataFromURL_doesNotDeliverResultAfterSUTInstanceHasBeenDeallocatid() {
