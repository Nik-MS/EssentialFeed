...
final class FeedLocalizationTests: XCTestCase {
    ...
    private func allLocalizedStringKeys(in bundles: [LocalizedBundle], table: String, file: StaticString = #file, line: UInt = #line) -> Set<String> {
        return bundles.reduce([]) { (partial, current) in
            guard let path = current.bundle.url(forResource: table, withExtension: "strings"),
                  let strings = NSDictionary(contentsOf: path),
                  let keys = strings.allKeys as? [String] else {
                XCTFail("Couldn't load localized strings for localization: \(current.localization)", file: file, line: line)
                return partial
            }
            
            return partial.union(Set(keys))
        }
    }
}
